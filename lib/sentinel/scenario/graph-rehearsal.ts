// Graph rehearsal scenario (W2.1 gate fixture) — the "test sequence in the
// scenario file [that] animates the full graph convincingly" the P2 gate
// asks for (brief §7). Loaded only via /sentinel?scenario=graph-rehearsal
// (app/sentinel/page.tsx) for a presenter to loop while dialing in the
// live-agent-graph renderer against a projector; it is never part of the
// audience demo and never reached by the default route.
//
// Unlike smoke-scenario.ts (player.test.ts's ≤50ms fixture, tuned for fast
...
// runtime at 1x is ~26-27s including narration's typing-effect overhead,
// comfortably inside the ~25-35s the gate asks for.
//
// No `awaitApproval`: the approval-resolution UI isn't wired until P3, so a
// real gate here would hard-block the rehearsal loop with no way to
// continue. The Approval Gate node is exercised directly instead, via
// explicit graphSteps, which is enough to prove its "gate pending" glow and
// its return to steady-lit. No `emitEvent`/`render`/`auditWrite`/
// `counterUpdate` either — this fixture rehearses only the graph and the
...
// distinct working -> done cycles with a visible gap, because the hero rule
// is cross-dataset), and a wind-down back to all-idle with edges cleared —
// proving idle dims correctly and edge-clearing works even though the
// player itself ends `done`. Narration between phases is one calm
// operational sentence each, in the v1 scripted voice (see
// lib/agents/payment-health/script.ts and smoke-scenario.ts's narration).
...
export const graphRehearsalScenario: SentinelScenario = {
  ...
  steps: [
...

    {
      ...
      text: 'Orchestrator is routing the servicing policy to Policy Analyst for parsing.',
    },
...

    {
      ...
      text:
        'Policy Analyst is citing a cross-dataset rule — Data Collector will pull both balance-transfer and payment history records.',
    },

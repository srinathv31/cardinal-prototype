// Servicing's DEMO_MODE=scripted state machine (W4.5, brief §7d — "the only
// live-model surface in the demo... every rehearsed question needs a
// checked-in script"). Same shape as lib/agents/ask/script.ts: keyword-based
// matching against a fixed set of rehearsed turns, one evidence call per
// question, a grounded takeaway built from this agent's own resolvers
// (resolvers.ts) — the same functions tools.ts's tools call — so narration
// is always byte-identical to what's on screen. Must NOT import ./agent
// (cycle risk — lib/ai/provider.ts's getAgentModel takes a script and
// agent.ts's `model:` line calls getAgentModel).
//
// Seven rehearsed turns total: the five §7b/Use-case-2 evidence kinds
// (transactions, next payment, next statement, balance, spending-by-category)
// plus the §7c contact change, plus the Use-case-3 card-activation gate.
//
// Persona pinning (DEMO_BUILD_PLAN.md D6, Wave 2 Agent E work item 1):
// like resolvers.ts and tools.ts, this file is now a factory —
// `createServicingScript(ctx)` — bound to one identity/persona for its
// whole lifetime, called once per agent construction
// (lib/agents/servicing/agent.ts). A script instance never depends on
// wall-clock or randomness (AgentScript's own contract), and rebuilding one
// fresh per request is exactly as cheap as the ToolLoopAgent it's attached
// to already is.

import { getPayments } from '@/lib/soe';
import type { AgentScript, ScriptStep } from '@/lib/ai/scripted/types';
import {
  lastUserMessageText,
  toolDisposition,
  toolResultsSinceLastUserMessage,
} from '@/lib/ai/scripted/types';
import { identityForPersona, type ServicingIdentity, type ServicingPersona } from './identity';
import { createServicingResolvers, type ServicingResolvers } from './resolvers';

type ServicingMatch =
  | 'transactions'
  | 'next-payment'
  | 'next-statement'
  | 'balance'
  | 'category'
  | 'contact'
  | 'activate-card'
  | 'none';

/** Keyword match against the rehearsed turns (brief §7b/§7c, Use case 3), in
 * a deliberate specificity order so e.g. "phone" never falls through to a
 * generic account question, and "next statement" never falls through to
 * "next payment" (both mention neither word exclusively, so "statement" is
 * checked as its own, earlier branch). Mirrors lib/agents/ask/script.ts's
 * matchAskQuestion. */
function matchServicingQuestion(text: string): ServicingMatch {
  const q = text.toLowerCase();
  if (q.includes('activate')) return 'activate-card';
  if (q.includes('phone') || q.includes('address')) return 'contact';
  if (q.includes('transaction') || q.includes('latest')) return 'transactions';
  if (q.includes('statement')) return 'next-statement';
  if (q.includes('payment') && (q.includes('due') || q.includes('next'))) return 'next-payment';
  if (q.includes('balance') || q.includes('available credit') || q.includes('credit limit')) return 'balance';
  if (q.includes('spend') || q.includes('spending') || q.includes('categor')) return 'category';
  return 'none';
}

// A fictional Austin, TX number consistent with the pinned cardholder's
// household (lib/soe/seed/patel.ts) — the fallback ONLY when the customer's
// own message doesn't contain a phone-shaped number, so a rehearsal that
// just says "I need to update my phone number" still has a concrete new
// value to propose (brief §3 Part B step 3 doesn't spell out a number).
const FALLBACK_PHONE = '(512) 555-0148';
const PHONE_PATTERN = /(\+?1[\s.-]?)?\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4}/;

/** Pure function of the customer's message text — never random, so the same
 * turn always proposes (and later confirms) the same number. */
function extractPhone(text: string): string {
  const match = PHONE_PATTERN.exec(text);
  return match ? match[0].trim() : FALLBACK_PHONE;
}

const CANT_HELP_NARRATION =
  "I can show your recent transactions, your next payment due, your next statement, your balance and available credit, or what you've been spending on by category — I can update the phone number or mailing address on file, and I can activate a new card. Try one of those.";

/** The tool's actual `{status, ...}` result read back from the resumed
 * prompt's own tool-result content — never re-derived via a second call to
 * lib/sentinel/activate-card.ts's activateCardForPersona, which would write
 * a second Event Log entry for the same approved action (CLAUDE.md 5e: one
 * write, once). Matches this file's convention elsewhere of trusting the
 * SDK-recorded result of the SAME execution rather than recomputing it. */
interface ActivateCardOutput {
  status?: string;
  confirmationId?: string;
  ruleId?: string;
  finding?: string;
}

function readActivateCardOutput(results: ReturnType<typeof toolResultsSinceLastUserMessage>): ActivateCardOutput | undefined {
  const match = results.find((r) => r.toolName === 'activateCard');
  if (!match || match.output.type !== 'json') return undefined;
  return match.output.value as ActivateCardOutput;
}

export interface ServicingScriptContext {
  identity: ServicingIdentity;
  persona: ServicingPersona;
  resolvers: ServicingResolvers;
}

/** Builds the servicing script bound to ONE request's identity/persona (see
 * this file's header). Call once per agent construction —
 * lib/agents/servicing/agent.ts's createServicingAgent is the only real
 * caller. */
export function createServicingScript(ctx: ServicingScriptContext): AgentScript {
  const { identity, resolvers } = ctx;

  return {
    agentId: 'servicing',

    async nextStep(prompt): Promise<ScriptStep> {
      const results = toolResultsSinceLastUserMessage(prompt);
      const questionText = lastUserMessageText(prompt) ?? '';
      const match = matchServicingQuestion(questionText);

      if (results.length === 0) {
        switch (match) {
          case 'transactions':
            return {
              narration: 'Pulling your recent transactions.',
              toolCalls: [
                {
                  toolName: 'renderEvidence',
                  input: {
                    component: 'TransactionTable',
                    source: { kind: 'servicing-recent-transactions', months: 3, limit: 15 },
                  },
                },
              ],
              done: false,
            };
          case 'next-payment':
            return {
              narration: 'Checking your next payment.',
              toolCalls: [
                {
                  toolName: 'renderEvidence',
                  input: { component: 'MetricRow', source: { kind: 'servicing-next-payment' } },
                },
              ],
              done: false,
            };
          case 'next-statement':
            return {
              narration: 'Checking your next statement.',
              toolCalls: [
                {
                  toolName: 'renderEvidence',
                  input: { component: 'MetricRow', source: { kind: 'servicing-next-statement' } },
                },
              ],
              done: false,
            };
          case 'balance':
            return {
              narration: 'Pulling your current balance and available credit.',
              toolCalls: [
                {
                  toolName: 'renderEvidence',
                  input: { component: 'MetricRow', source: { kind: 'servicing-account-summary' } },
                },
              ],
              done: false,
            };
          case 'category':
            return {
              narration: "Checking what you've been spending on.",
              toolCalls: [
                {
                  toolName: 'renderEvidence',
                  input: { component: 'CategoryPie', source: { kind: 'servicing-category-spend', months: 3 } },
                },
              ],
              done: false,
            };
          case 'contact': {
            const phone = extractPhone(questionText);
            return {
              narration: `I can update the phone number on file to ${phone} — confirming with you before I make the change.`,
              toolCalls: [
                {
                  toolName: 'updateContactInfo',
                  input: {
                    phone,
                    rationale: `Customer asked to update the phone number on file to ${phone}.`,
                  },
                },
              ],
              done: false,
            };
          }
          case 'activate-card':
            return {
              narration: 'I can activate that card for you — confirming with you before I run it.',
              toolCalls: [
                {
                  toolName: 'activateCard',
                  input: { rationale: 'Activate the card on file for your account.' },
                },
              ],
              done: false,
            };
          case 'none':
            return { narration: CANT_HELP_NARRATION, toolCalls: [], done: true };
        }
      }

      // Second pass — the tool this turn called already has a result.
      if (match === 'contact') {
        const disposition = toolDisposition(results, 'updateContactInfo');
        const phone = extractPhone(questionText);
        const narration =
          disposition === 'approved'
            ? `Done — the phone number on file is now ${phone}.`
            : 'No problem — I did not make that change. The phone number on file is unchanged.';
        return { narration, toolCalls: [], done: true };
      }

      if (match === 'activate-card') {
        const disposition = toolDisposition(results, 'activateCard');
        if (disposition === 'denied') {
          return { narration: 'No problem — I did not activate the card.', toolCalls: [], done: true };
        }
        const output = readActivateCardOutput(results);
        if (output?.status === 'blocked') {
          return {
            narration: `Your card arrived, but this account is currently failing a policy — ${output.finding ?? ''}`.trim(),
            toolCalls: [],
            done: true,
          };
        }
        if (output?.status === 'activated') {
          return {
            narration: `Your card is activated. Confirmation ${output.confirmationId ?? ''}.`.trim(),
            toolCalls: [],
            done: true,
          };
        }
        return { narration: 'That covers your card activation.', toolCalls: [], done: true };
      }

      if (match === 'transactions') {
        const instruction = await resolvers.resolveRecentTransactions(3, 15);
        if (instruction.component !== 'TransactionTable') {
          throw new Error('servicing script: unexpected evidence shape while building the transactions takeaway');
        }
        return {
          narration: instruction.props.footnote ?? 'That covers your recent transactions.',
          toolCalls: [],
          done: true,
        };
      }

      if (match === 'next-payment') {
        // getPayments, not the resolver's own recomputation, so this stays a
        // fresh SOE read independent of the resolver (same spirit as
        // lib/agents/payment-health/script.ts's missedPaymentDueDate helper).
        const scheduled = (await getPayments(identity.accountId)).find((p) => p.status === 'SCHEDULED');
        const instruction = await resolvers.resolveNextPayment();
        if (instruction.component !== 'MetricRow') {
          throw new Error('servicing script: unexpected evidence shape while building the next-payment takeaway');
        }
        const dueDate = instruction.props.metrics.find((m) => m.label === 'Due Date')?.value;
        const amountDue = instruction.props.metrics.find((m) => m.label === 'Amount Due')?.value;
        const narration =
          scheduled && dueDate && amountDue
            ? `Your next payment of ${amountDue} is due ${dueDate}.`
            : 'That covers your next payment.';
        return { narration, toolCalls: [], done: true };
      }

      if (match === 'next-statement') {
        const scheduled = (await getPayments(identity.accountId)).find((p) => p.status === 'SCHEDULED');
        const instruction = await resolvers.resolveNextStatement();
        if (instruction.component !== 'MetricRow') {
          throw new Error('servicing script: unexpected evidence shape while building the next-statement takeaway');
        }
        const statementBalance = instruction.props.metrics.find((m) => m.label === 'Statement Balance')?.value;
        const dueDate = instruction.props.metrics.find((m) => m.label === 'Due Date')?.value;
        const narration =
          scheduled && statementBalance && dueDate
            ? `Your next statement is ${statementBalance}, due ${dueDate}.`
            : 'That covers your next statement.';
        return { narration, toolCalls: [], done: true };
      }

      if (match === 'balance') {
        const instruction = await resolvers.resolveAccountSummary();
        if (instruction.component !== 'MetricRow') {
          throw new Error('servicing script: unexpected evidence shape while building the balance takeaway');
        }
        const balance = instruction.props.metrics.find((m) => m.label === 'Balance')?.value;
        const available = instruction.props.metrics.find((m) => m.label === 'Available Credit')?.value;
        const narration =
          balance && available
            ? `Your balance is ${balance}, with ${available} available.`
            : 'That covers your balance and available credit.';
        return { narration, toolCalls: [], done: true };
      }

      if (match === 'category') {
        const instruction = await resolvers.resolveCategorySpend(3);
        if (instruction.component !== 'CategoryPie') {
          throw new Error('servicing script: unexpected evidence shape while building the category takeaway');
        }
        const top = instruction.props.slices[0];
        const totalValue = instruction.props.total?.value;
        const narration =
          top && totalValue
            ? `${top.label} leads at ${top.share} of your trailing spend, totaling ${totalValue}.`
            : totalValue
              ? `Your trailing spend totals ${totalValue}.`
              : "That covers what you've been spending on.";
        return { narration, toolCalls: [], done: true };
      }

      // evidenceCount > 0 with match === 'none' is unreachable — the only way
      // to get a tool result is this same script proposing one on a match.
      // Stays terminal and truthful if it's ever hit anyway.
      return { narration: 'That covers what I found.', toolCalls: [], done: true };
    },
  };
}

// Back-compat singleton (Wave 2 Agent E persona-pinning refactor) — bound to
// the 'happy' persona (Anand Patel), exactly this module's
// pre-persona-pinning behavior. lib/agents/scripts.test.ts's cross-agent
// script suite still imports `servicingScript` this way; new code (and
// lib/agents/servicing/agent.ts's createServicingAgent) should prefer
// calling createServicingScript directly with the request's own resolved
// identity/persona.
export const servicingScript: AgentScript = createServicingScript({
  identity: identityForPersona('happy'),
  persona: 'happy',
  resolvers: createServicingResolvers(identityForPersona('happy')),
});
